...
func (r *ShimV2OCIRuntime) ExecContainer(c *Container, sessionID string, options *ExecOptions, streams *define.AttachStreams, newSize *resize.TerminalSize) (int, chan error, error) {
	...
	pid, err := readShimV2PipeData(r.name, pipes.syncPipe, ociLog)
...
func (r *ShimV2OCIRuntime) ExecContainerHTTP(ctr *Container, sessionID string, options *ExecOptions, req *http.Request, w http.ResponseWriter,
	...
	attachChan := make(chan error)
	shimV2PipeDataChan := make(chan shimV2PipeData)
	go func() {
		// attachToExec is responsible for closing pipes
		attachChan <- attachExecHTTP(ctr, sessionID, req, w, streams, pipes, detachKeys, options.Terminal, cancel, hijackDone, holdConnOpen, execCmd, shimV2PipeDataChan, ociLog, newSize, r.name)
		close(attachChan)
	}()
	...
	pipeData := <-shimV2PipeDataChan

	return pipeData.pid, attachChan, pipeData.err
}

// shimV2PipeData contains the data when reading from shimV2's pipe.
type shimV2PipeData struct {
	pid int
	err error
}
...
func (r *ShimV2OCIRuntime) ExecContainerDetached(ctr *Container, sessionID string, options *ExecOptions, stdin bool) (int, error) {
	...
	// to proceed.
	if _, err := readShimV2PipeData(r.name, pipes.attachPipe, ""); err != nil {
		return -1, err
	}

	// Start the exec session
	if err := writeShimV2PipeData(pipes.startPipe); err != nil {
		return -1, err
	}
...
func attachExecHTTP(c *Container, sessionID string, r *http.Request, w http.ResponseWriter, streams *HTTPAttachStreams, pipes *execPipes, detachKeys []byte, isTerminal bool, cancel <-chan bool, hijackDone chan<- bool, holdConnOpen <-chan bool, execCmd *exec.Cmd, shimV2PipeDataChan chan<- shimV2PipeData, ociLog string, newSize *resize.TerminalSize, runtimeName string) (deferredErr error) {
	...
	// shimV2PipeDataChan in case of an error.
	...
	if pipes == nil || pipes.startPipe == nil || pipes.attachPipe == nil {
		...
		shimV2PipeDataChan <- shimV2PipeData{-1, err}
		return err
	}
	...
	if _, err := readShimV2PipeData(runtimeName, pipes.attachPipe, ""); err != nil {
		shimV2PipeDataChan <- shimV2PipeData{-1, err}
		return err
	}
	...
	go func() {
		...
		if err := execCmd.Wait(); err != nil {
			...
			pid, err := readShimV2PipeData(runtimeName, pipes.syncPipe, ociLog)
			if err != nil {
				hijackWriteError(err, c.ID(), isTerminal, httpBuf)
				shimV2PipeDataChan <- shimV2PipeData{pid, err}
			} else {
				shimV2PipeDataChan <- shimV2PipeData{pid, err}
			}
		}
